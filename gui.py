    ...
    if event == "Exit" or event == sg.WIN_CLOSED:
        ...
        if len(stations) == 1:
            try:
                ...
                img = Image.open(str(fn)).convert("RGBA")
                size = (int(w/3+w/3)-int(0.02*w), int(0.67*h)-int(0.04*h))
                img = img.resize(size, resample=Image.BICUBIC)
                img.save(str(images_data_folder / 'temp.png'),'PNG')
                window["-IMAGE_L-"].update(str(images_data_folder / 'temp.png'))
                
                fn = images_data_folder / '%s.PNG' %(stations[0])
                size = (int(w/3.5), int(h/3)-int(0.01*h))
                img_w = Image.open(str(fn)).convert("RGBA")
                img_w = img_w.resize(size, resample=Image.BICUBIC)
                img_w.save(str(images_data_folder / 'temp.png'),'PNG')
                window["-IMAGE_TR-"].update(str(images_data_folder / 'temp.png'))
            ...
            try:
                fn = dayplot_data_folder / ('DAYPLOT_DATE_20101216_STATION_%s_COMPONENT_%s_FILTER_%s_%s%s.PNG' %(stations[0],comp,filt,prep,zoom))
                #bg_img = Image.open(str(fn)).convert("RGBA")
                bg_img = cv2.imread(str(fn))
                for ii in range(1,len(stations)):
                    ...
                    #fg_img = Image.open(str(fn)).convert("RGBA")
                    fg_img = cv2.imread(str(fn))
                    if ii == 1:
                ...
                cv2.imwrite(str(dayplot_data_folder / 'temp.png'),new_img)
                new_img = Image.open(str(dayplot_data_folder / 'temp.png')).convert("RGBA")
                size = (int(w/3+w/3)-int(0.01*w), int(0.67*h)-int(0.04*h))
                new_img = new_img.resize(size, resample=Image.BICUBIC)
                new_img.save(str(dayplot_data_folder / 'temp.png'),'PNG')
                ...
                window["-IMAGE_L-"].update(str(dayplot_data_folder / 'temp.png'))
                if len(stations) < 5:
